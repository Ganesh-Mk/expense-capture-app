...
import { Loader2, Upload, X, AlertTriangle, CheckCircle2, Info, ReceiptText } from 'lucide-react'
...
export default function NewExpensePage() {
  ...
  useEffect(() => {
    if (!watchAmount || !watchCategory || !watchDate || !user || !profile) return
  ...
  async function runValidation(amount: number, categoryId: string, date: string) {
  ...
  function validateForm(data: FormData): boolean {
    ...
    if (!data.amount || isNaN(amt) || amt <= 0) errs.amount = 'Amount must be greater than 0'
    if (amt > 999999) errs.amount = 'Amount too large'
    if (!data.description || data.description.length < 3) errs.description = 'Provide a brief description'
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    if (file.size > 5 * 1024 * 1024) { setApiError('Receipt file must be under 5 MB'); return }
    setReceiptFile(file)
  ...
  async function onSubmit(data: FormData) {
    ...
    if (validation?.status === 'rejected') {
      setApiError('This expense exceeds your allowed limits and cannot be submitted.')
      return
    }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <CardContent className="grid gap-4 px-5 py-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <NativeSelect id="category_id" aria-invalid={!!fieldErrors.category_id} value={field.value ?? ''} onChange={field.onChange}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="expense_date" className="text-xs font-medium">Date *</Label>
                <Input id="expense_date" type="date" aria-invalid={!!fieldErrors.expense_date} {...register('expense_date')} />
                {fieldErrors.expense_date && <p className="text-xs text-destructive">{fieldErrors.expense_date}</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="merchant_name" className="text-xs font-medium">Merchant / Vendor</Label>
                <Input id="merchant_name" placeholder="e.g. Swiggy, OYO" {...register('merchant_name')} />
              </div>
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="location" className="text-xs font-medium">Location</Label>
                <Input id="location" placeholder="e.g. Mumbai, Delhi" {...register('location')} />
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <>
                {validation.rule && (
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { label: 'Daily limit', value: `₹${validation.rule.daily_limit.toFixed(0)}` },
                      { label: 'Spent today', value: `₹${validation.rule.daily_spent.toFixed(0)}` },
                      { label: 'Remaining', value: `₹${validation.rule.daily_remaining.toFixed(0)}` },
                    ].map(stat => (
                      <div key={stat.label} className="rounded-md border bg-muted/30 px-3 py-2 text-center">
                ...
                {validation.status === 'rejected' && (
                  <Alert variant="destructive">
                    ...
                    <AlertTitle className="text-xs font-semibold">Expense rejected</AlertTitle>
                    <AlertDescription className="text-xs">
                      <ul className="mt-1 space-y-0.5">{validation.violations.map((v, i) => <li key={i}>• {v}</li>)}</ul>
                    </AlertDescription>
                  </Alert>
                )}
                ...
                {validation.status === 'flagged' && (
                  <Alert className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/30">
                    ...
                    <AlertTitle className="text-xs font-semibold text-amber-800 dark:text-amber-200">Flagged for review</AlertTitle>
                    <AlertDescription className="text-xs text-amber-700 dark:text-amber-300">
                      <ul className="mt-1 space-y-0.5">{validation.violations.map((v, i) => <li key={i}>• {v}</li>)}</ul>
                      <p className="mt-1">The expense will be submitted but requires manager approval.</p>
                    </AlertDescription>
                  </Alert>
                )}
        ...
        <Card className="gap-0 py-0">
          <CardHeader className="border-b px-5 py-4">
            ...
            <CardDescription className="text-xs">Upload an image or PDF (max 5 MB)</CardDescription>
          </CardHeader>
            ...
            {receiptFile ? (
              <div className="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  {receiptPreview ? (
                    <img src={receiptPreview} alt="Receipt" className="size-10 rounded object-cover border" />
                  ) : (
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-xs font-medium">{receiptFile.name}</p>
                    <p className="text-[10px] text-muted-foreground">{(receiptFile.size / 1024).toFixed(0)} KB</p>
                  </div>
                </div>
                <Button type="button" variant="ghost" size="icon-sm" onClick={() => { setReceiptFile(null); setReceiptPreview(null) }}>
        ...
        <div className="flex gap-3">
          ...
          <Button type="submit" disabled={submitting || validation?.status === 'rejected'} className="flex-1">
            {submitting && <Loader2 className="size-4 animate-spin" />}
            {!submitting && validation?.status === 'flagged' && (
              <Badge variant="outline" className="mr-1 text-[10px]">Flagged</Badge>
            )}
        ...
        <div className="flex items-start gap-1.5 rounded-md bg-muted/50 px-3 py-2">
          ...
          <p className="text-[10px] text-muted-foreground">
            Expenses are validated against Grade {profile?.grade} policies. Flagged expenses require manager approval.
          </p>
        </div>
      </form>
    </div>
